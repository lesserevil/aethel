...
Implementation note, June 5, 2026: the committed MVP runtime assets use
Kenney Furniture Kit GLBs for desk, chair, laptop, keyboard, monitor, trash
can, desk lamp, and book stack. Coffee cup and notebook are deterministic
local low-poly GLBs generated by
`scripts/assets/populate-office-runtime-glbs.mjs` and catalogued as CC0
Aethel generated office props. Eclair remains a future supplemental source for
richer clutter, but it is not the source of the committed runtime GLBs.
...
- GLB structure tests that parse committed runtime assets and fail on empty
  placeholder GLBs with no meshes or binary payload;
...
- [ ] CRIT-1: A documented free asset selection exists for the standard
      ...
      test.
- [ ] CRIT-2: The web MVP loads mesh-backed office GLB assets from
      `web/public/assets/office/` through a typed manifest, rejects empty
      placeholder GLBs in tests, and preserves deterministic fallback
      rendering when asset loading is mocked or an asset fails.
